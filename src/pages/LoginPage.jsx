...
export default function LoginPage() {
  ...
  return <div className="login-screen"><div className="login-card">
    ...
    <form onSubmit={submit} className="login-form">
      <label>Email<input type="email" required={!demo} value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="owner@business.com" /></label>
      <label>Password<input type="password" required={!demo} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="••••••••" /></label>
      {error && <div className="alert danger">{error}</div>}
      <button className="primary-btn" disabled={busy}>{busy ? 'Signing in…' : demo ? 'Open demo workspace' : 'Sign in'}</button>
    </form>
